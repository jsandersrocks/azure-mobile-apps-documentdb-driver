...
const defaults = {
    connectionString: process.env['CUSTOMCONNSTR_MS_DocumentDbConnectionString'] || null,
...
function configureDriver (configuration) {
    log.verbose(`[docdbdriver] configureDriver.  Configuration = ${JSON.stringify(configuration, null, 2)}`);
    driverSettings = Object.assign({}, defaults, configuration);
    if (typeof driverSettings.connectionString === 'string' && driverSettings.connectionString !== null) {
        let re = new RegExp(/^AccountEndpoint=([^;]+);AccountKey=([^;]+);/);
        let results = re.exec(driverSettings.connectionString);
        if (results !== null) {
            configuration.host = results[1];
            configuration.accountKey = results[2];
        }
    }

    log.verbose(`[docdbdriver] driver config = ${JSON.stringify(driverSettings, null, 2)}`);
    ...
    log.verbose(`[documentdb-driver] dbClient = `, dbClient);
}

/**
 * Initialize the table so that it is "warmed up" and ready for use.
 * 1) Create appropriate schema if necessary
 * 2) Insert items into the table specified by the seed property of the table configuration
...
function initializeTable (table) {
    log.verbose(`[docdbdriver] initializing table.  config = ${JSON.stringify(table, null, 2)}`);

    return ops.ensureDatabaseExists(dbClient, driverSettings.database)
        .then((databaseRef) => {
            log.verbose(`[docdbdriver] Initialized database ${driverSettings.database}.  Reference = ${JSON.stringify(databaseRef, null, 2)}`);
        ...
        .then((cref) => {
            log.verbose(`[docdbdriver] Initialized collection: reference = ${JSON.stringify(cref, null, 2)}`);
...
function getSchema (table) {
    log.verbose(`[docdbdriver] getSchema for tableConfig = ${JSON.stringify(table, null, 2)}`);
...
function truncateTable (table) {
    log.verbose(`[docdbdriver] truncateTable for tableConfig = ${JSON.stringify(table, null, 2)}`);
...
function readOperation (table, query) {
    log.verbose(`[docdbdriver/readOperation] table=${table.name}: `);
...
function insertOperation (table, item) {
    log.verbose(`[docdbdriver] insertOperation table=${table.name}: ${JSON.stringify(item, null, 2)}`);
    ...
    .then((doc) => {
        log.verbose(`[docdbdriver/insertOperation] inserted item ${JSON.stringify(doc)}`);
...
function updateOperation (table, item, query) {
    log.verbose(`[docdbdriver] updateOperation table=${table.name}: ${JSON.stringify(item, null, 2)}`);

    let readPromise;
    if (typeof query === 'undefined') {
        log.verbose(`[docbdriver/updateOperation]: doing read of explicit ID`);
        let querySpec = {
        ...
        documents.forEach((v) => {
            if (v.id === item.id) {
                if (item.hasOwnProperty('version')) {
                    let etag = Buffer.from(item.version, 'base64').toString('ascii');
                    if (etag === v._etag) {
...
function deleteOperation (table, query, version) {
    log.verbose(`[docdbdriver] deleteOperation table=${table.name}: ${JSON.stringify(query, null, 2)}`);
    let sqlCommands = convertQueryToSql(table, query);
    return ops.queryDocuments(dbClient, refs.tables[table.name], sqlCommands[0])
    .then((documents) => {
        let deletedRecords = [];
        ...
        documents.forEach((v) => {
            if (typeof version !== 'undefined') {
                let etag = Buffer.from(version, 'base64').toString('ascii');
                if (etag !== v._etag) {
            ...
            if (table.softDelete === true) {
                ...
                updatePromises.push(ops.deleteDocument(dbClient, v._self));
            }
...
function undeleteOperation (table, query, version) {
    log.verbose(`[docdbdriver] undeleteOperation table=${table.name}: ${JSON.stringify(query, null, 2)}`);
    let sqlCommands = convertQueryToSql(table, query);
    return ops.queryDocuments(dbClient, refs.tables[table.name], sqlCommands[0]).then((documents) => {
        let restoredRecords = [];
        let updatePromises = [];
...
function convertItemForTransmission (v) {
    if (v.hasOwnProperty('_etag')) {
        v.version = Buffer.from(v._etag).toString('base64');
        delete v._etag;
    }
...
function convertQueryToSql (table, query) {
    ...
    log.verbose(`[docdbdriver/convertQueryToSql] odataQuery (before)=${JSON.stringify(odataQuery, null, 2)}`);
    ...
    log.verbose(`[docdbdriver/convertQueryToSql] odataQuery (after)=${JSON.stringify(odataQuery, null, 2)}`);

    let sqlCommands = formatSql(odataQuery, table);
    log.verbose(`[docdbdriver/convertQueryToSql] sqlCommands=${JSON.stringify(sqlCommands, null, 2)}`);
