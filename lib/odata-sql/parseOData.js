...
var ODataParser = types.defineClass(ctor, {
    ...
    _parseComparison: function () {
        ...
        while (this.token.id === 'Equal' || this.token.id === 'NotEqual' || this.token.id === 'GreaterThan' || this.token.id === 'GreaterThanEqual' || this.token.id === 'LessThan' || this.token.id === 'LessThanEqual') {
    ...
    _parseMappedFunction: function (mappedMember, errorPos) {
        ...
        if (this.token.id === 'OpenParen') {
            ...
            if (!mappedMember.isStatic) {
                if (args.length === 0) {
                    throw this._parseError(util.format("No applicable method '%s' exists in type '%s'", mappedMember.memberName, mappedMember.type), errorPos);
                }
    ...
    _validateFunctionParameters: function (functionName, args, expectedArgCount) {
        if (args.length !== expectedArgCount) {
            throw new Error(`Function '${functionName}' requires ${expectedArgCount} parameter(s)`);
        }
    },
